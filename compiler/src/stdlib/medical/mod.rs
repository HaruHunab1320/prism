...
pub fn create_medical_module() -> Module {
    ...
    module.register_function("validate_symptom", Value::NativeFunction(Arc::new(|_interpreter: &mut Interpreter, args: Vec<Value>| {
        if args.len() != 1 {
            return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput, "validate_symptom takes exactly 1 argument")));
        }
        let symptom = match &args[0] {
            Value::String(s) => s.clone(),
            _ => return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput, "validate_symptom requires a string argument"))),
        };
    ...
    module.register_function("semantic_match", Value::NativeFunction(Arc::new(|_interpreter: &mut Interpreter, args: Vec<Value>| {
        if args.len() != 2 {
            return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput, "semantic_match takes exactly 2 arguments")));
        }
        let (symptom, disease) = match (&args[0], &args[1]) {
            (Value::String(s), Value::String(d)) => (s.clone(), d.clone()),
            _ => return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput, "semantic_match requires two string arguments"))),
        };
    ...
    module.register_function("get_disease_pattern", Value::NativeFunction(Arc::new(|_interpreter: &mut Interpreter, args: Vec<Value>| {
        if args.len() != 1 {
            return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput, "get_disease_pattern takes exactly 1 argument")));
        }
        let disease = match &args[0] {
            Value::String(d) => d.clone(),
            _ => return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput, "get_disease_pattern requires a string argument"))),
        };
